...
matchSchedule = json.load(open(myPath + "/matchSchedule_PRACTICE.json","r"))
...
class Panel(wx.Panel):
    def __init__(self, parent):
        ...
        self.autonPlatformTitle = wx.StaticText(self, label="PLATFORM Zone Cubes", pos=(20, 220))
        self.autonPlatformInput = wx.TextCtrl(self, pos=(220, 220), size=(30,20))
        self.autonPlatformInput.AppendText("0")
        self.autonPlatformInputUp = wx.Button(self, label="+", pos=(270, 217.5), size=(25,25), name="autonPlatformInputUp")
        self.autonPlatformInputDown = wx.Button(self, label="-", pos=(300, 217.5), size=(25,25), name="autonPlatformInputDown")
        ...
        self.autonExchangeTitle = wx.StaticText(self, label="EXCHANGE Cubes", pos=(20, 180))
        self.autonExchangeInput = wx.TextCtrl(self, pos=(220, 180), size=(30,20))
        ...
        self.robotPosition = wx.RadioBox(self, pos=(420,30), majorDimension = 2, choices=["9","9","Red far                         ","Blue far","Red mid","Blue mid","Red near","Blue near"], style = wx.RA_SPECIFY_COLS)
        self.robotPosition.ShowItem(0, False)
        self.robotPosition.ShowItem(1, False)
        self.robotPosition.SetSelection(0)
        self.switchlabel = wx.StaticText(self, label="Switch Alliance sides", pos=(700, 60))
        ...
        png = wx.Image(myPath + "/FieldPic.png", wx.BITMAP_TYPE_ANY).ConvertToBitmap()
        png.SetSize((170,170))
        wx.StaticBitmap(self, -1, png, (430, 200), ((png.GetWidth()), (png.GetHeight())))
        self.question = wx.StaticText(self, label="Where was the robot when it was placing the cube?", pos=(420, 190))
        ...
        self.teleOpExchangeTitle = wx.StaticText(self, label="EXCHANGE Cubes", pos=(20, 330))
    ...
    def Red_Right(self, event):
        if self.redRight.GetValue():
            self.robotPosition.SetItemLabel(2, "Blue far                         ")
            self.robotPosition.SetItemLabel(3, "Red far")
            self.robotPosition.SetItemLabel(4, "Blue mid")
            self.robotPosition.SetItemLabel(5, "Red mid")
            self.robotPosition.SetItemLabel(6, "Blue near")
            self.robotPosition.SetItemLabel(7, "Red near")
            self.robotPosition.SetItemLabel(0, "9")
            self.robotPosition.SetItemLabel(1, "9")
        else:
            self.robotPosition.SetItemLabel(2, "Red far                         ")
            self.robotPosition.SetItemLabel(3, "Blue far")
            self.robotPosition.SetItemLabel(4, "Red mid")
            self.robotPosition.SetItemLabel(5, "Blue mid")
            self.robotPosition.SetItemLabel(6, "Red near")
            self.robotPosition.SetItemLabel(7, "Blue near")
            self.robotPosition.SetItemLabel(0, "9")
            self.robotPosition.SetItemLabel(1, "9")
        event.Skip()
    ...
    def CSV_OUTPUT(self, event):
        ...
        if self.robotPosition.GetItemLabel(self.robotPosition.GetSelection()) != "9":
            ...
            if self.robotPosition.GetSelection() == 2:
                autonPosition = "Red1"
            elif self.robotPosition.GetSelection() == 3:
                autonPosition = "Blue3"
            elif self.robotPosition.GetSelection() == 4:
                ...
                autonPosition = "Blue2"
            elif self.robotPosition.GetSelection() == 6:
                autonPosition = "Red3"
            elif self.robotPosition.GetSelection() == 7:
                autonPosition = "Blue1"
        else:
            autonPosition = ""

        autonScaleFrom = ""
        if self.null and self.plat:
            autonScaleFrom = "Front, Side"
        elif self.null:
            autonScaleFrom = "Side"
        elif self.plat:
            autonScaleFrom = "Front"

        if not Path(myPath + "/ScoutingData", filename + '.csv').exists():
            with open(os.path.join(myPath + "/ScoutingData", filename + '.csv'), 'w') as csvfile:
                fieldnames = ['name','matchNumber','teamNumber', 'deadRobot', 'autonPosition','baseline','autonSwitch','autonScale','autonSwitchColor', 'autonScaleColor','autonPZ','autonPCZ','autonExchange','autonScaleFrom','teleopSwitch','teleopScale','teleopExchange','teleopOppoSwitch','robotOnPlatform','robotClimb','buddyBar','strategies','penalties', 'scaleBot', 'switchBot', 'climbBot', 'exchangeBot', 'defenseBot', 'helpingOthersClimbBot', 'othersBot', 'comments','timeStamp']

                writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
                writer.writeheader()

                writer.writerow({'name':self.nameInput.GetValue(), 'matchNumber':self.matchNumInput.GetValue(), 'teamNumber':teamNum, 'deadRobot':self.deadRobot.GetValue(), 'autonPosition':autonPosition, 'baseline':self.baselineInput.GetValue(), 'autonSwitch':self.autonSwitchInput.GetValue(), 'autonScale':self.autonScaleInput.GetValue(), 'autonSwitchColor':switchColor, 'autonScaleColor':scaleColor, 'autonPZ':self.autonPlatformInput.GetValue(), 'autonPCZ':self.autonPowerCubeInput.GetValue(), 'autonExchange':self.autonExchangeInput.GetValue(), 'autonScaleFrom':autonScaleFrom, 'teleopSwitch':self.teleOpSwitchInput.GetValue(), 'teleopScale':self.teleOpScaleInput.GetValue(), 'teleopExchange':self.teleOpExchangeInput.GetValue(), 'teleopOppoSwitch':self.teleOpOppoSwitchInput.GetValue(), 'robotOnPlatform':self.teleOpParked.GetSelection(), 'robotClimb':self.teleOpClimbed.GetSelection(), 'buddyBar':self.teleOpTeamwork.GetSelection(), 'strategies':self.strategyInput.GetValue(), 'penalties':self.penaltyInput.GetValue(), 'scaleBot':self.isRobotScale.GetValue(), 'switchBot':self.isRobotSwitch.GetValue(), 'climbBot':self.isRobotClimb.GetValue(), 'exchangeBot':self.isRobotExchange.GetValue(), 'defenseBot':self.isRobotDefense.GetValue(), 'helpingOthersClimbBot':self.isRobotHelpingOthers.GetValue(), 'othersBot':self.isRobotOthersInput.GetValue(), 'comments':self.commentsInput.GetValue(),'timeStamp':datetime.now().time()})
        else:
        ...
        self.matchNumInput.AppendText(str(matchNum + 1))
        self.teamNumInput.Clear()
        ...
        self.plat.SetValue(0)
        self.null.SetValue(0)
        ...
        copyfile(os.path.join(myPath + "/ScoutingData", filename + '.csv'), os.path.join(ScoutingDir, filename + '.csv'))
        # print filename
